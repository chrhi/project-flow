    ...
    <div className='w-[70%]  h-full flex justify-start items-center gap-x-6 '>
      {LIST.map((item , index ) => (
           ...
           onPromisClick={() => handleRouting(item.path , item.page , index - 1)}
                className={`${buttonVariantsAbdullah({variant :'ghost' , size:"sm" })} text-gray-500  ${current_page === item.page ? 'bg-blue-100 text-slate-900' : null} text-xs font-poppins font-semibold `}
           >
           {item.name}
           </AbdullahEffectButton>
      ))}
