...
export  function ConfirmePopUpDeleteStakeHolder ({refetch} : Props) {

  const isShowing = confirmDeleteStakeholder(state => state.showModel)

  const setIsShowing = confirmDeleteStakeholder(state => state.setShowModel)

  const UserId = confirmDeleteStakeholder(state => state.id)

  const mutation = api.StakeHolderRouter.remove_stakeholder.useMutation({
    onSuccess : async  () => {
      toast.success("stakeholder has been deleted")
      closeModal()
      await refetch()
    },
    onError(){
